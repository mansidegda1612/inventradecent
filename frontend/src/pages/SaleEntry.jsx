...
export default function SaleEntry() {
  ...
  const PrepareData = async (row) => {
    ...
    // roundoff are re-derived from the sequence below.
    ...
    let obj = {
      ...
      items: rawItems.map((i) => {
        ...
        const amounts = calcItemAmounts(itemBase, isgstbill, gstFactor);
  ...
  const open = async (row) => {
    ...
    // Fetch products and capture the result for use below
  ...
  const save = async () => {
    ...
    // totals.expenses carries the resolved GST / roundoff amounts, and
    // totals.items the per-line GST charged on the post-discount base.
    const totals = calcTotals(form.items, form.expenses, form.isGSTBill);
  ...
  const updateExpense = (index, field, value) => {
    setForm((f) => {
      ...
      // % is charged on the amount this expense sits on in the sequence — for
      // the first expense that's the item amount, for a later one it's the
      // running total after everything before it.
  ...
  const handleGSTBillChange = (v) => {
    setForm((f) => ({
      ...
      items: f.items.map((item) => {
        const amounts = calcItemAmounts(item, v); // recalc with new flag
        return { ...item, ...amounts };
      }),
    }));
  };
 ...
 const handleWhatsApp = async (focused) => {
  ...
  return (
    <div>
      <PageHeader title="Sales Entry" sub="Create and manage sales invoices" />

      {/* ── DataGrid listing — UNCHANGED ─────────────────────────────────── */}
      <Card noPad>
        <DataGrid
          title=""
          dateFilter={true}
      ...
      >
        <div className="tr-layout">
          ...
          {/* ── TOP: Transaction Header + Customer inline ─────────────────── */}
          <TransactionHeader
            type={"SI"}
          ...
          <div className="tr-toolbar">
            <div className="tr-toolbar-field">
              <div className="tr-toolbar-label">Barcode / Product Code</div>
              <input
                ref={barRef}
                autoFocus
